...
export default function EditReplyPopup({rClean, rConvin, rFaci, rFood, rService, rWorth, rRating, rTitle, rComment, rid, hid} 
    : {rClean:boolean, rConvin:boolean, rFaci:boolean, rFood:boolean, rService:boolean, rWorth:boolean, rRating:number, rTitle:string, rComment:string, rid:string, hid:string}){
    ...
    <>
      ...
      <div className="h-[100%] w-[100%] rounded-xl mx-auto bg-white shadow-sm relative p-5 mb-[20px]"> 
          ...
          disabled:bg-slate-300 disabled:text-slate-500 disabled:shadow-none
           ...
           onClick={()=>router.push(`/hotels/${hid}`)}
          >Cancel</button>
